...
5. Add the following toleration to deploy correctly on autopilot:
...
6. If using an existing K8s secret for sensitive Falcon data, make sure your secret name is `falcon-node-sensor-secret`.

```yaml
falconSecret:
  enabled: true
  namespace: <falcon-secret-namespace>
  secretName: falcon-node-sensor-secret
```
